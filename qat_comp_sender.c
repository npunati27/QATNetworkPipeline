...
typedef struct {
  ...
  volatile uint64_t socket_blocked_count[NUM_SENDERS];
...
ssize_t read_chunk_from_files(file_reader_t *reader, uint8_t *buffer,
                              size_t chunk_size) {
  ...
  while (total_read < chunk_size) {
    ...
    size_t bytes_read =
        fread(buffer + total_read, 1, to_read, reader->current_file);
...
void qat_dc_callback(void *pCallbackTag, CpaStatus status) {
  ...
  if (status == CPA_STATUS_SUCCESS) {
    ...
    __sync_fetch_and_add(&g_ring->qat_stats.total_bytes_out,
                         entry->producedSize);
...
void *producer_thread(void *arg) {
  ...
  while (ring->running) {
    ...
    // ssize_t bytes_read = read_chunk_from_files(&ring->file_reader,
    ...
    if (next_tail != __atomic_load_n(&ring->consumer.head, __ATOMIC_ACQUIRE) &&
      ...
      // INFO_PRINT("REACHED!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
      //  if (__atomic_compare_exchange_n(&ring->producer.tail, &current_tail,
      //  next_tail,
    ...
    if (u_next_tail != __atomic_load_n(&queue->head, __ATOMIC_ACQUIRE)) {
      if (__atomic_compare_exchange_n(&queue->tail, &u_current_tail,
        ...
        u_entry->sequence_number =
            __sync_fetch_and_add(&ring->sequence_counter, 1);
    ...
    if (now - last_report >= 5) {
      ...
      INFO_PRINT("Producer %d: %.3f Gbps | RingFull: %lu | QATBusy: %lu | "
                 "QueueFull: %lu\n",
                 thread_id, gbps, ring_full_count, qat_backpressure_count,
                 queue_full_count);
  ...
  while (ring->running) {
    ...
    if (current_status == JOB_COMPLETED) {
      ...
      if (next_tail == queue->head) {
        ...
        // after a while change the status to empty and move to next compressed
        // send queue entry
      ...
      //     if (leaks++ % 100 == 0) INFO_PRINT("Consumer: Mitigated leaked slot
      //     at %u\n", current_head);
      // }
...
/* NETWORK THREAD
    * if there is data in compressed send queue, send first --> then fallback to
   uncompressed queue

*/
// BATCH_SIZE for Round-Robin scheduling
  ...
  while (ring->running) {
    ...
    if (now - last_report_time >= 5) {
      uint64_t current_packets =
          ring->compressed_packets_sent + ring->uncompressed_packets_sent;
      uint64_t packets_per_sec =
          (current_packets - last_packets_sent) / (now - last_report_time);

      // periodic debugging output
      DEBUG_PRINT("Network: %lu packets/sec (comp:%lu uncomp:%lu "
                  "queued_comp:%u queued_uncomp:%u)\n",
                  packets_per_sec, ring->compressed_packets_sent,
                  ring->uncompressed_packets_sent,
                  (ring->compressed_send_queue.tail -
                   ring->compressed_send_queue.head) &
                      ring->compressed_send_queue.mask,
                  (ring->uncompressed_send_queue.tail -
    ...
    int comp_processed = 0;

    uint32_t current_head, next_head;
    int batch_size = 0;

    do {
      current_head = __atomic_load_n(&comp_queue->head, __ATOMIC_ACQUIRE);
      uint32_t current_tail =
          __atomic_load_n(&comp_queue->tail, __ATOMIC_ACQUIRE);
      if (current_head == current_tail) {
        batch_size = 0;
        break;
      }

      int available = (current_tail - current_head) & comp_queue->mask;
      batch_size = (available > BATCH_SIZE) ? BATCH_SIZE : available;
      next_head = (current_head + batch_size) & comp_queue->mask;

    } while (!__atomic_compare_exchange_n(&comp_queue->head, &current_head,
                                          next_head, true, __ATOMIC_ACQ_REL,
                                          __ATOMIC_ACQUIRE));

    if (batch_size > 0) {
      for (int i = 0; i < batch_size; i++) {
        uint32_t idx = (current_head + i) & comp_queue->mask;
        compressed_send_entry_t *send_msg = &comp_queue->entries[idx];
        ring_entry_t *entry = send_msg->entry;

        if (ring->socket_fd[args->thread_id] > 0) {
          struct iovec iov[2];
          ...
          while (total_sent < total_to_send) {
            ssize_t sent =
                writev(ring->socket_fd[args->thread_id], current_iov, 2);
            ...
            if (sent > 0) {
              ...
              if (sent >= current_iov[0].iov_len) {
                ...
                current_iov[0].iov_base =
                    (uint8_t *)current_iov[0].iov_base + sent;
                current_iov[0].iov_len -= sent;
              }
            } else if (sent < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
              __sync_fetch_and_add(&ring->socket_blocked_count[args->thread_id],
                                   1);
          ...
          if (total_sent == total_to_send) {
            ...
            __sync_fetch_and_add(&ring->network_bytes_in,
    ...
    // Process UNCOMPRESSED queue (BATCHED)
    uncompressed_send_queue_t *uncomp_queue = &ring->uncompressed_send_queue;
    batch_size = 0;
    int uncomp_processed = 0;

    do {
      current_head = __atomic_load_n(&uncomp_queue->head, __ATOMIC_ACQUIRE);
      uint32_t current_tail =
          __atomic_load_n(&uncomp_queue->tail, __ATOMIC_ACQUIRE);
      if (current_head == current_tail) {
        batch_size = 0;
        break;
      }

      int available = (current_tail - current_head) & uncomp_queue->mask;
      batch_size = (available > BATCH_SIZE) ? BATCH_SIZE : available;
      next_head = (current_head + batch_size) & uncomp_queue->mask;

    } while (!__atomic_compare_exchange_n(&uncomp_queue->head, &current_head,
                                          next_head, true, __ATOMIC_ACQ_REL,
                                          __ATOMIC_ACQUIRE));

    if (batch_size > 0) {
      for (int i = 0; i < batch_size; i++) {
        uint32_t idx = (current_head + i) & uncomp_queue->mask;
        uncompressed_send_entry_t *u_entry = &uncomp_queue->entries[idx];
        ...
        if (ring->socket_fd[args->thread_id] > 0) {
          ...
          ssize_t total_to_send =
              u_entry->length + sizeof(compression_header_t);
          ssize_t total_sent = 0;
          ...
          if (total_sent == total_to_send) {
            ...
            if (uncomp_sent_entries % 10000 == 0)
              DEBUG_PRINT(
                  "Network Send Thread: Sent %lu uncompressed entries\n",
                  uncomp_sent_entries);
          } else {
          }
        }
...
void *monitor_thread(void *arg) {
  ...
  while (ring->running) {
    ...
    uint32_t ring_occupancy =
        (ring->producer.tail - ring->consumer.head) & ring->mask;
    uint32_t comp_occupancy =
        (ring->compressed_send_queue.tail - ring->compressed_send_queue.head) &
        ring->compressed_send_queue.mask;
    ...
    INFO_PRINT("Queue Occupancy: Ring=%u/%d, Comp=%u/%d, Uncomp=%u/%d\n",
               ring_occupancy, RING_SIZE, comp_occupancy, SEND_QUEUE_SIZE,
...
int setup_listener(int port) {
  ...
  if (listen(sockfd, NUM_SENDERS) < 0) {
...
void print_qat_stats(ring_buffer_t *ring, double elapsed_sec) {
  if (ring->qat_stats.total_compressions > 0) {
    ...
    //                             ring->qat_stats.first_submit_time.tv_nsec) /
    //                             1e9;

    // double mb_processed = (double)ring->qat_stats.total_bytes_in / (1024 *
    // 1024); double mb_bytes_out = (double)ring->qat_stats.total_bytes_out /
    // (1024 * 1024); double throughput_mbps = mb_processed / elapsed_sec;
    ...
    // ring->qat_stats.total_compressions); printf("  Average compression time:
    // %.2f µs\n", avg_time); printf("  Compression ratio: %.2f%%\n",
    // compression_ratio * 100); printf("  Operations per second: %.0f\n",
    // ops_per_sec); printf("  Throughput: %.2f MB/s\n", throughput_mbps);
...
const char *map_file_number(const char *input) {
  static const char *base_path =
      "/home/npunati2/SquashBench/"; // TODO: change to where squash benchmarks
                                     // are located
...
int main(int argc, char *argv[]) {
  ...
  ring_buffer_t *ring =
      ring_buffer_init(compression_level, compressed_fraction, input_file);
  if (!ring) {
  ...
  if (listen_fd < 0) {
    printf("Fatal: Failed to setup listener on port %d\n", port);
    return 1;
  }

  for (int i = 0; i < NUM_SENDERS; i++) {
    ring->socket_fd[i] = accept_connection(listen_fd);
    if (ring->socket_fd[i] < 0) {
  ...
  monitor_args->core_id =
      numa_node2_base + NUM_PRODUCERS + NUM_CONSUMERS + NUM_SENDERS;
  ...
  double app_gbps =
      (ring->producer.app_bytes_generated * 8.0) / (elapsed_sec * 1e9);
