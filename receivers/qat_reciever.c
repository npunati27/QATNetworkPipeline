...
void *network_thread(void *arg) {
  ...
  while (g_ring->running) {
    ...
    if (header.magic == MAGIC_COMPRESSED) {
      ...
      do {
        current_tail =
            __atomic_load_n(&g_ring->producer.tail, __ATOMIC_ACQUIRE);
        next_tail = (current_tail + 1) & g_ring->mask;

        if (next_tail ==
            __atomic_load_n(&g_ring->consumer.head, __ATOMIC_ACQUIRE)) {
        ...
        entry = &g_ring->entries[current_tail];
        ...
        if (__atomic_load_n(&entry->status, __ATOMIC_ACQUIRE) != JOB_EMPTY) {
          ...
          &g_ring->producer.tail, &current_tail, next_tail, true,
    ...
    skip_compressed:
      ...
      __sync_fetch_and_add(&g_ring->producer.packets_received, 1);
      __sync_fetch_and_add(&g_ring->uncompressed_bytes,
  ...
  while (ring->running && !g_shutdown_requested) {
    ...
    if (elapsed > 0) {
      ...
      printf("RX: %.3f Gbps (wire) | Decompressed: %.3f Gbps | "
             ...
             rx_gbps, decomp_gbps, pkt_delta / elapsed, queue_depth,
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < NUM_RECEIVERS; i++) {
    ...
    printf("Connecting thread %d to %s:%d...\n", i, host, port);
  ...
  pthread_t net_tids[NUM_RECEIVERS], stats_tid;
  pthread_t *worker_tids = malloc(num_workers * sizeof(pthread_t));

  for (int i = 0; i < NUM_RECEIVERS; i++) {
    pthread_create(&net_tids[i], NULL, network_thread, &sockfds[i]);
  }
  ...
  double network_in_gbps =
      (ring->producer.bytes_received * 8.0) / (elapsed * 1e9);
  double qat_tput_in = (ring->qat_stats.total_bytes_in * 8.0) / (elapsed * 1e9);
  double qat_tput_out =
      (ring->qat_stats.total_bytes_out * 8.0) / (elapsed * 1e9);
